  ...
  "ResolveStringPattern" should {
    "return no errors" when {
      "given a matching string" in {
        val result = resolveTaxYearPattern("2026-27")
        result shouldBe Valid("2026-27")
      }

      "given a matching string in an Option" in {
        val result = resolveTaxYearPattern(Option("2026-27"))
        result shouldBe Valid(Some("2026-27"))
      }
      ...
      "given a matching string via the legacy apply() function" in {
        val result = ResolveStringPattern("2026-27", taxYearRegex, TaxYearFormatError)
        result shouldBe Valid("2026-27")
      }

      "given a matching string in an Option via the legacy apply() function" in {
        val result = ResolveStringPattern(Option("2026-27"), taxYearRegex, TaxYearFormatError)
        result shouldBe Valid(Some("2026-27"))
      }
