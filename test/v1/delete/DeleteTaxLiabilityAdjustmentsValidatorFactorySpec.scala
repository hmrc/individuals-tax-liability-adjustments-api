...
class DeleteTaxLiabilityAdjustmentsValidatorFactorySpec extends UnitSpec {

  private def validatorFor(taxYear: String) =
    new DeleteTaxLiabilityAdjustmentsValidatorFactory().validator(
      nino = "ignoredNino",
      taxYear = taxYear
    )

  "DeleteTaxLiabilityAdjustmentsValidatorFactory" when {
    "given a request corresponding to a Def1 schema" should {
      "return a Def1 validator" in {
        validatorFor("2026-27") shouldBe a[Def1_DeleteTaxLiabilityAdjustmentsValidator]
      }
    }
